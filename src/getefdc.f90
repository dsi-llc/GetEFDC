...
! ** NDLuan
! ** UPDATE: 2018-12-04
! ** UPDATE BELV, BDELTA and EE_BED format 084: 2019-02-25
...
SDFILEI = trim(OUTFOLDER)//'EE_SD.OUT'

RES = CHANGEDIRQQ(INPFOLDER)

CALL SKIPCOM(UINP, '*')
READ(UINP,*) LAYK, JULTIME, NLOC, ROT, INDEX, VPROFILE,TECOPT,ZOPT,NDRIFTER,LAYBED
IF (VPROFILE>=1) THEN
  VPROFCHK =.TRUE.
ELSE
  VPROFCHK =.FALSE.
...
CALL EFDCPRO
IF( LAYBED > KB )THEN
  PRINT*,'*** BED LAYER INDEX MUST BE LESS THAN THE TOTAL NUMBER OF SEDIMENT BED LAYERS KB'
ENDIF
...
ELSE
  ...
  UOUTI(100:150) = (/(500+N,N=100,150)/)
  UOUTI(150:200) = (/(500+N,N=150,200)/)
  
  ! ** OUTPUT FILES FOR THE SELECTED LAYER:
  WSFILEO = '#output\RESULT\DEP_TSK_DOM.DAT'
  TSWSF   = '#output\RESULT\DEP_TSK_CEL.DAT'
  TSICF   = '#output\RESULT\ICE_TSK_CEL.DAT'
  ...
  ELSEIF(LAYK==0) THEN  
    VEFILEO = '#output\RESULT\VEL_TSK_AVG_DOM.DAT'
    TSVEF   = '#output\RESULT\VEL_TSK_AVG_CEL.DAT'
  ENDIF
  ...
  CALL GETEE_WS
  IF( LAYK > 0 )THEN
    PRINT*,'*** READING EE_BC.OUT AND EXPORTING DATA '
    CALL GETEE_BC
  ENDIF
  IF( LAYK/=-1 )THEN
  ...
  INQUIRE(FILE = 'EE_TUR.OUT',EXIST = RES)
  IF( RES )THEN
  ...
  IF( ANY(ISTRAN(1:7) == 1) .AND. LAYK /= -1 )THEN  
  ...
  IF( ISTRAN(8) == 1 .AND. LAYK /= -1) THEN
    PRINT*,'*** READING EE_WQ.OUT AND EXPORTING DATA '
    CALL GETEE_WQ
    INQUIRE(FILE= SDFILEI,EXIST=RES)
    IF( RES )THEN 
      PRINT*,'*** READING EE_SD.OUT AND EXPORTING DATA '
      CALL GETEE_SD
    ENDIF
  ENDIF
